...
def print_version():
    ...
    logger.info('作者: MrAshers | opensecurity.in')
    if platform.system() == 'Windows':
        logger.info('AndroMSF %s', ver)
        print('REST API Key: ' + api_key())
    else:
        logger.info('\033[1m\033[34mAndroMSF %s\033[0m', ver)
    ...
    if dist:
        dst_str = f' ({dist}) '
    env_str = f'操作系统: {os}{dst_str}{pltfm}'
    logger.info(env_str)
    find_java_binary()
    check_basic_env()
...
def check_basic_env():
    ...
    logger.info('AndroMSF Basic Environment Check')
    try:
